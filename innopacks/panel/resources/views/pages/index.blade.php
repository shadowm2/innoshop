                            ...
                            @foreach ($pages as $item)
                                <tr>
                                    ...
                                    <td><a href="{{ $item->url }}" target="_blank"
                                            class="text-decoration-none">{{ $item->fallbackName('title') }}</a></td>
                                    <td>{{ $item->slug }}</td>
                                    <td>{{ $item->viewed }}</td>
                                    <td>@include('panel::shared.list_switch', [
                                        'value' => $item->active,
                                        'url' => panel_route('pages.active', $item->id),
                                    ])</td>
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="{{ panel_route('pages.edit', [$item->id]) }}">
                                                <el-button size="small" plain
                                                    type="primary">{{ __('panel/common.edit') }}</el-button>
                                            </a>
                                            <form ref="deleteForm" action="{{ panel_route('pages.destroy', [$item->id]) }}"
