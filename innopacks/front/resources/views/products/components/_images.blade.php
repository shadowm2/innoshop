@if(is_array($product->images) || $product->video)
  ...
  <div class="sub-product-img d-none d-lg-block">
    <div class="swiper" id="sub-product-img-swiper">
      <div class="swiper-wrapper" id="product-desktop-gallery">
          ...
          @foreach($product->images as $image)
            <div class="swiper-slide">
              <div class="thumbnail-item border border-2 border-transparent rounded d-flex align-items-center justify-content-center"
                   ...
                   style="width: 100px; height: 100px; overflow: hidden;">
                <a href="{{ image_resize($image, 1200, 1200) }}"
                   data-pswp-width="1200"
                   data-pswp-height="1200">
                  <img src="{{ image_resize($image, 100, 100) }}"
                       class="img-fluid"
...
<!-- Desktop main image -->
<div class="main-product-img d-none d-lg-block position-relative w-100 overflow-hidden bg-light"
     style="aspect-ratio: 1/1;" >
  @hookinsert('front.product.show.image.before')


  <img src="{{ $product->image_url }}" class="img-fluid main-image w-100 h-100" style="object-fit: cover;" alt="">
            ...
            <div class="swiper-slide">
              <div class="position-relative w-100 h-100">
                <a href="{{ image_resize($image, 1200, 1200) }}"
                   data-pswp-width="1200"
                   data-pswp-height="1200">
                  <img src="{{ image_resize($image, 600, 600) }}"
                       class="img-fluid w-100 h-100"
                       style="object-fit: cover;"
                       alt=""
                  >
...
@push('footer')
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      // Desktop PhotoSwipe
      if (document.querySelector('#product-desktop-gallery')) {
        const desktopLightbox = new PhotoSwipeLightbox({
          gallery: '#product-desktop-gallery',
          children: 'a',
          pswpModule: PhotoSwipe
        });
        desktopLightbox.init();
        $(".main-image").on('click', () => {
          desktopLightbox.loadAndOpen()
        })
      }

      // Mobile PhotoSwipe
      if (document.querySelector('#mobile-product-swiper')) {
        const mobileLightbox = new PhotoSwipeLightbox({
          gallery: '#mobile-product-swiper',
          children: 'a',
          pswpModule: PhotoSwipe
        });
        mobileLightbox.init();
