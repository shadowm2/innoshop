                            ...
                            <div class="addresses-wrap">
                                <div class="shipping-address">
                                    <div class="title-wrap">
                                        ...
                                        <div>
                                            <span class="cursor-pointer" v-if="!source.addressEdit"
                                            ...
                                            <div class="left">
                                                ...
                                                <div class="select-title">
                                                    ...
                                                    <div class="address-info">@{{ address.address_1 }} @{{ address.address_2 }}
                                                        {{ __('common/address.city') }}: @{{ address.city }},
                                                        {{ __('common/address.state') }}: @{{ address.state }},
                                                        {{ __('common/address.country') }}: @{{ address.country_name }}
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div>
                                            <label class="form-check-label" v-if="!source.addressEdit">
                                                <input class="form-check-input" type="checkbox"
                                                    v-model="source.same_as_shipping_address">
                                                ...
                                                <div class="left">
                                                    ...
                                                    <div class="select-title">
                                                        ...
                                                        <div class="address-info">@{{ address.address_1 }}
                                                            @{{ address.address_2 }} {{ __('common/address.city') }}:
                                                            @{{ address.city }}, {{ __('common/address.state') }}:
                                                            @{{ address.state }}, {{ __('common/address.country') }}:
                        ...
                        <div v-show="source.addressEdit">
                            <div class="checkout-item">
                                <div class="title-wrap">
                                    ...
                                    @endif
                                    <span class="cursor-pointer" v-if="source.addresses.length"
                                        @click="addressEdit(false)"><i class="bi bi-plus-lg"></i>
                    ...
                    <div class="checkout-item">
                        ...
                        <div class="checkout-select-wrap">
                            ...
                            <div v-if="!source.billingMethods.length" class="alert alert-warning"><i
                                ...
                                {{ __('front/checkout.no_billing_methods') }}</div>
                        </div>
                    </div>
                                                    ...
                                                    <div class="product-info">
                                                        ...
                                                        <div class="sku mt-2 text-secondary">{{ $product['sku_code'] }}
                                                            ...
                                                            @if ($product['item_type_label'])
                                                                <span
                                                                    class="badge bg-danger ms-2">{{ $product['item_type_label'] }}</span>
                                                            @endif
                                                            x {{ $product['quantity'] }}
                                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-12 d-flex align-items-center gap-3">
                                        <div class="input-group flex-nowrap">
                                            <span
                                                class="input-group-text">{{ default_currency() ? default_currency()->symbol_left : '' }}</span>
                                            <input type="text" v-model="current.balance" class="form-control py-2"
                                                placeholder="{{ __('front/transaction.balance_placeholder') }}"
                                                aria-label="{{ __('front/transaction.balance') }}"
                                                @input="validateInput">
                                        </div>
                                        <button
                                            :class="{
                                                'disabled': parseFloat(current.balance) > source.balanceAmount ||
                                                    parseFloat(current
                                                        .balance) >= source.totalAmount || isNaN(parseFloat(current
                                                        .balance))
                                            }"
                                            class="input-group-text btn btn-primary py-2" id="addon-wrapping"
                                            @click="submitBalance"
        ...
        const checkoutApp = createApp({
            setup() {
                const source = reactive({
                    ...
                    totalAmountFormat: @json($amount !== null ? currency_format($amount) : '0'),
                ...
                const isCheckout = computed(() => {
                    return !current.shipping_address_id || !current.billing_address_id || !current
                ...
                const editAddress = (index) => {
                    ...
                    address_form.find('select[name="country_code"]').val(address.country_code).trigger(
                        'change');
                    setTimeout(() => {
                        address_form.find('select[name="state_code"]').val(address.state_code).trigger(
                            'change');
                        setTimeout(() => {
                    ...
                    axios.post(api.checkoutConfirm, current).then(function(res) {
                        if (res.success) {
                            layer.msg(res.message, {
                                ...
                                location.href = inno.getBase() + '/orders/' + res.data.number +
                                    '/pay'
                            })
                        }
                    }).finally(function() {
                ...
                const submitBalance = () => {
                    if (parseFloat(current.balance) <= source.balanceAmount && parseFloat(current.balance) <
                        source
                        .totalAmount) {
