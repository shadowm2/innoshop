    ...
    <div class="login-register-box {{ request('iframe') ? 'iframe' : '' }}">
      ...
      <form action="{{ front_route('register.store') }}" class="needs-validation form-wrap" novalidate>
        ...
        <div class="btn-submit">
          <button type="button"
            class="btn btn-primary form-submit btn-lg">{{ __('front/register.register_submit') }}</button>
          <a href="{{ front_route('login.index') }}{{ request('iframe') ? '?iframe=true' : '' }}">{{ __('front/register.have_account') }}
