<template id="link-selector">
    <div class="link-selector-wrap">
        <div class="selector-type" @blur="selectorContentShow = false" tabindex="1">
            <div class="title" v-if="!link.type || link.type === '' || !value.value" @click="toggleSelector">لطفاً نوع
                لینک را
                انتخاب کنید
            </div>
            ...
            <div class="link-dialog-content">
                <div class="product-search">
                    ...
                    <a :href="linkTypeAdmin" target="_blank"
                        v-if="link.type != 'custom' && link.type != 'static'">مدیریت
                        @{{ dialogTitle }}</a>
                            ...
                            <ul class="product-list static">
                                <li v-for="(product, index) in static" @click="link.value = product.value">
                                    <div class="content-cell">
                                        <span
                                            :class="'radio-plus ' + (link.value == product.value ? 'active' : '')"></span>
                ...
                <template v-else>
                    <div class="product-info" v-loading="loading">
                        <template v-if="linkDialog.data.length">
                            ...
                            <ul class="product-list">
                                <li v-for="(product, index) in linkDialog.data"
                                    ...
                                    <div class="content-cell">
                                        <span
                                            :class="'radio-plus ' + (link.value == product.id ? 'active' : '') + (!product
                                                .active ? 'no-status' :
                                                '')"></span>
                        ...
                        <div class="product-info-no" v-if="!linkDialog.data.length && loading === false">
                            ...
                            <div class="no-text">داده وجود ندارد یا حذف شده است، <a :href="linkTypeAdmin"
                                    target="_blank">برای
        ...
        methods: {
            ...
            updateData() {
                ...
                this.link = {
                    ...
                    new_window: this.value?.new_window !== undefined ? this.value.new_window : (this
                        .linkNew || false),
